...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      frameAncestors: ["'self'", "https://sdk.cashfree.com"],
      formAction: ["'self'", "https://api.cashfree.com", "https://sdk.cashfree.com"],
      objectSrc: ["'none'"],
    },
  },
...
const ADMIN_PAGE_VERSION = process.env.ADMIN_PAGE_VERSION || '20260613a';
...
app.post('/api/polls', fcmLimiter, requireAnyAuth, [
  ...
  try {
    ...
    console.log(`[poll] Poll created by ${req.userEmail}: "${question.trim()}" (${formattedOptions.length} options, ${duration_type})`);
